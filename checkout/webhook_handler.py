...
class StripeWH_Handler:
    ...
    def __send__confirmation_email(self, order):
        """Send the user a confirmation email"""
        cust_email = order.email
        subject = render_to_string(
            'checkout/confirmation__emails/confirmation_email_subject.txt',
            {'order': order})
        body = render_to_string(
            'checkout/confirmation__emails/confirmation_email_body.txt',
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        while attempt <= 5:
            try:
                ...
                time.sleep(1)
        if order_exists:
            self.__send__confirmation_email(order)
            return HttpResponse(
            ...
            try:
                ...
                for item_id, item_data in json.loads(bag).items():
                    ...
                    if isinstance(item_data, int):
                        ...
                        for quantity in item_data.items():  # noqa
                            order_line_item = OrderLineItem(
                                order=order,
                                product=product,
                                quantity=quantity,
